...
# =============================================================================
#
# FILE: HOOMD_convex_polyhedron_compression_with_fixed_move_size.py
# VERSION: 1
...
# =============================================================================
#
# This script performs systematic compression of a hard convex polyhedron system from an
...
#   - Complete overlap removal at each step (hard particle constraint)
...
# =============================================================================
#
# 1. HPMC POLYHEDRA INTEGRATOR:
#    - Implements hard convex polyhedron Monte Carlo moves
...
# ASSUMPTIONS:
# -----------
# 1. MONODISPERSE SYSTEM: All particles are identical
#
...
#
# 3. HARD POTENTIAL: Purely geometric constraint
...
#    - Only monodisperse hard polyhedra supported
...
class SimulationParams:
    ...
    This dataclass represents the complete configuration of a hard-polyhedra
    ...
    Attributes:
        ...
        input_gsd_filename (str):
            Path to the input GSD file containing the initial hard-polyhedra
            configuration. 
    ...
    # --- Convex-polyhedron shape definition ---
    ...
    # ------------------------------------------------------------------
    # Runtime-resolved convex-polyhedron metadata
    # ------------------------------------------------------------------

    particle_volume: Optional[float] = None  # Actual scaled particle volume
    shape_vertices: Optional[list] = None    # Scaled vertices used by HPMC
    shape_name: Optional[str] = None         # Human-readable shape name
    shape_short_name: Optional[str] = None   # Short shape label
...
def load_simulparams(json_path: str) -> SimulationParams:
    ...
    for float_field in ["target_pf", "volume_scaling_factor", "move_size_translation", "move_size_rotation"]:
...
def resolve_filenames(params: SimulationParams) -> RunFiles:
    ...
    # -----------------------------------------------------------------------
        ...
        if Path(final_gsd).exists():
            sys.exit(
                ...
                f"              This indicates stage {sid} has already completed.\n"
                f"              To run the next stage:\n"
                f"                1. Edit JSON: set stage_id_current to {sid + 1}\n"
    ...
    # -----------------------------------------------------------------------
        ...
        sys.exit(
            f"[FATAL ERROR] Invalid stage_id_current: {sid}\n"
...
# ===========================================================================
#
# For a one-component system of identical convex polyhedra:
#
#     phi = (N * V_particle) / V_box
#
# where V_particle is the actual volume of the scaled polyhedron.
#
#     V_particle = V_reference * scale^3
#
# ===========================================================================

def packing_fraction(N: int, particle_volume: float, box: hoomd.Box) -> float:
    """
    Calculate the packing fraction of a one-component convex-polyhedron system.
    ...
    particle_volume : float
        Actual volume of one particle after scaling the reference shape.
    box : hoomd.Box
        Current HOOMD simulation box.

    Returns
    -------
    float
        Packing fraction phi = N * V_particle / V_box.
    """
    return N * float(particle_volume) / float(box.volume)

def load_convex_polyhedron_shape_from_json(shape_json_filename: str,
                                           scale: float) -> tuple[str, str, float, list]:
    """
    Load the reference convex-polyhedron shape JSON, validate it, and return the
    scaled vertices together with the scaled particle volume.

    Expected JSON keys
    ------------------
    "1_Name"     : human-readable shape name
    "2_ShortName": short shape label
    "4_volume"   : reference shape volume
    "8_vertices" : reference vertices in the body/principal frame
    ...
    ------
    SystemExit
        If the file is missing, malformed, or physically invalid.
    """
    ...
    except json.JSONDecodeError as shape_json_error:
        sys.exit(
            f"[FATAL ERROR] Invalid JSON syntax in shape file '{shape_json_filename}': "
            f"line {shape_json_error.lineno}, column {shape_json_error.colno}\n"
        )
    except Exception as shape_open_error:
        sys.exit(
            f"[FATAL ERROR] Failed to open shape JSON '{shape_json_filename}': "
            f"{shape_open_error}\n"
        )
    ...
    try:
        ...
        sys.exit(
            ...
            f"'{shape_json_filename}': {vertex_error}\n"
        )
    ...
    if scale <= 0.0:
        sys.exit(
            f"[FATAL ERROR] shape_scale must be > 0, got {scale}\n"
        )
...
def validate_polyhedron_input_gsd(gsd_filename: str,
                                  expected_vertices: Optional[list] = None) -> None:
    """
    Validate that the input GSD is compatible with a one-component convex-
    polyhedron HPMC simulation.

    This function checks:
      1. the file exists and contains at least one frame,
      2. the particle count is non-zero,
      3. all particles belong to a single type,
      4. particle orientations are present and have shape (N, 4),
      5. if type_shapes are present in the GSD, they are compatible with the
         expected convex-polyhedron vertices.

    Notes
    -----
    For anisotropic particles, orientations matter physically. A missing or
    malformed orientation array is therefore treated as a fatal input error.
    """
    path = Path(gsd_filename)

    if not path.exists():
        sys.exit(
            f"[FATAL ERROR] Input GSD file not found: {gsd_filename}\n"
        )
    ...
    try:
        with gsd.hoomd.open(name=str(path), mode="r") as traj:
            if len(traj) == 0:
                sys.exit(
                    f"[FATAL ERROR] Input GSD '{gsd_filename}' has zero frames.\n"
                )
            frame = traj[-1]
    except Exception as gsd_error:
        sys.exit(
            f"[FATAL ERROR] Failed to open input GSD '{gsd_filename}': {gsd_error}\n"
        )

    N = int(frame.particles.N)
    if N <= 0:
        sys.exit(
            f"[FATAL ERROR] Input GSD '{gsd_filename}' contains zero particles.\n"
        )

    typeid = np.asarray(frame.particles.typeid, dtype=np.int64)
    if typeid.size != N:
        sys.exit(
            f"[FATAL ERROR] Input GSD '{gsd_filename}' has inconsistent typeid length.\n"
        )

    unique_typeids = np.unique(typeid)
    if unique_typeids.size != 1 or int(unique_typeids[0]) != 0:
        sys.exit(
            f"[FATAL ERROR] This compression script assumes a one-component system "
            f"with particle typeid = 0 only.\n"
            f"              Found unique typeids: {unique_typeids.tolist()}\n"
        )

    orientation = np.asarray(frame.particles.orientation, dtype=np.float64)
    if orientation.shape != (N, 4):
        sys.exit(
            f"[FATAL ERROR] Input GSD '{gsd_filename}' does not contain a valid "
            f"(N, 4) particle orientation array.\n"
            f"              Found shape: {orientation.shape}\n"
        )

    if not np.all(np.isfinite(orientation)):
        sys.exit(
            f"[FATAL ERROR] Input GSD '{gsd_filename}' contains non-finite "
    ...
    if np.any(norms <= 0.0):
        sys.exit(
            f"[FATAL ERROR] Input GSD '{gsd_filename}' contains zero-norm quaternions.\n"
        )

    try:
        gsd_type_shapes = frame.particles.type_shapes
    except Exception:
        gsd_type_shapes = None

    if expected_vertices is not None and gsd_type_shapes:
        try:
            first_shape = gsd_type_shapes[0]
            gsd_vertices = np.asarray(first_shape["vertices"], dtype=np.float64)
            ...
            sys.exit(
                f"[FATAL ERROR] Failed while validating GSD type_shapes: {compare_error}\n"
            )
...
def build_simulation(
    ...
    files: RunFiles,
    seed: int,
) -> tuple[hoomd.Simulation, hoomd.hpmc.integrate.ConvexPolyhedron]:
    """
    Build and fully configure a HOOMD-blue v4 convex-polyhedron compression simulation.
    ...
    Returns:
        tuple[hoomd.Simulation, hoomd.hpmc.integrate.ConvexPolyhedron]:
            ...
            - mc: HPMC ConvexPolyhedron integrator, provides overlaps and move stats
    ...
    # -----------------------------------------------------------------------
    # STEP 4: Read convex-polyhedron metadata
    # -----------------------------------------------------------------------
    root_print("\n[BUILD] Step 4/9: Reading convex-polyhedron metadata...")
    ...
    try:
        ...
        validate_polyhedron_input_gsd(
            state_source_gsd,
            expected_vertices=params.shape_vertices,
        )

        root_print(
            f"[BUILD] Shape loaded successfully:\n"
            ...
            f"        - Number of vertices: {len(params.shape_vertices)}"
        )
    ...
    phi_initial = packing_fraction(N, params.particle_volume, sim.state.box)
    root_print(
        f"[BUILD] Initial box: "
    ...
    # ------------------------------------------------------------------
    #  HPMC ConvexPolyhedron integrator  (FIXED move sizes — no tuner)
    # ------------------------------------------------------------------
    root_print("\n[BUILD] Step 6/9: Configuring HPMC ConvexPolyhedron integrator...")
    ...
    try:
        ...
        root_print(
            f"[BUILD] HPMC ConvexPolyhedron configured:\n"
            f"        - Shape type:         ConvexPolyhedron\n"
            f"        - Shape name:         {params.shape_name}\n"
            f"        - Vertex count:       {len(params.shape_vertices)}\n"
            f"        - Move size d:        {params.move_size_translation} (fixed)\n"
            f"        - Move size a:        {params.move_size_rotation} (fixed)\n"
            f"        - Trials per sweep:   {N} (nselect=1)"
        )

    except Exception as hpmc_error:
        sys.exit(
            f"[FATAL ERROR] HPMC ConvexPolyhedron integrator configuration failed: "
            f"{hpmc_error}\n"
            f"              => Check HOOMD version, shape vertices, and move-size values."
        )
    ...
    class _PhiLogger:
        ...
        def __init__(self, sim_ref, n, particle_volume):
            self._sim = sim_ref
            self._N = int(n)
        ...
        def packing_fraction(self) -> float:
            try:
                return packing_fraction(self._N, self._particle_volume, self._sim.state.box)
            except Exception:
                return 0.0
    ...
    class _RotateAcceptanceLogger:
        """Logger for rotational move acceptance rate."""

        _export_dict = {"rotate_acceptance": ("scalar", True)}

        def __init__(self, mc_ref):
            self._mc = mc_ref

        @property
        def rotate_acceptance(self) -> float:
            try:
                moves = self._mc.rotate_moves
                total = moves[0] + moves[1]
                return float(moves[0]) / float(total) if total > 0 else 0.0
            except hoomd.error.DataAccessError:
                return 0.0
    ...
    try:
        phi_logger        = _PhiLogger(sim, N, params.particle_volume)
        overlap_logger    = _OverlapLogger(mc)
        acceptance_logger = _AcceptanceLogger(mc)
        rotate_accept_logger = _RotateAcceptanceLogger(mc)
        ...
        root_print(
            ...
            f"        - rotate_acceptance (from mc.rotate_moves)"
        )        
    ...
    try:
        ...
        gsd_logger = hoomd.logging.Logger(categories=["scalar", "string", "sequence", "object"])
        
        # Add built-in HOOMD quantities
        gsd_logger.add(sim, quantities=["timestep", "tps", "walltime"])
        
        # Add HPMC integrator quantities
        gsd_logger.add(mc, quantities=["type_shapes", "translate_moves", "rotate_moves"])
        ...
        gsd_logger[("compression", "rotate_acceptance")] = (rotate_accept_logger, "rotate_acceptance", "scalar")

        root_print(
            f"[BUILD] GSD logger configured:\n"
            f"        - Built-in: timestep, tps, walltime\n"
            f"        - HPMC: type_shapes, translate_moves\n"
            f"        - Custom: packing_fraction, overlap_count, translate_acceptance, rotate_acceptance"
        )
    ...
    try:
        ...
        root_print(
            ...
            f"        - Shape metadata: logged from mc.type_shapes"
        )
    ...
    try:
        ...
        table_logger.add(mc, quantities=["translate_moves", "rotate_moves"])
        ...
        table_logger[("compression", "rotate_acceptance")] = (rotate_accept_logger, "rotate_acceptance", "scalar")
        ...
        root_print(
            ...
            f"        - Columns: timestep, tps, translate_moves, rotate_moves, "
            f"phi, overlaps, t_accept, r_accept"
        )
...
# KEY INVARIANTS:
# --------------
# 1. After each inner loop: mc.overlaps == 0 
...
def run_compression(
    ...
    Args:
        ...
        mc (hoomd.hpmc.integrate.ConvexPolyhedron): HPMC integrator attached to sim.
    ...
    try:
        volume_target = N * particle_volume / pf_target
    except ZeroDivisionError:
        sys.exit(
        ...
        sys.exit(
            ...
            f"              N={N}, particle_volume={particle_volume}, target_pf={pf_target}\n"
            f"              => Check that parameters are reasonable."
        )


    box_initial  = sim.state.box
    phi_initial  = packing_fraction(N, particle_volume, box_initial)
    ...
    if initial_overlaps > 0:
        ...
        root_print(f"[INFO] Initial configuration: 0 overlaps (valid hard-polyhedron state)")
    ...
    try:
        while sim.state.box.volume > volume_target:
            ...
            phi_current = packing_fraction(N, particle_volume, sim.state.box)
            outer_step += 1
            ...
            while overlaps > 0:
                ...
                if inner_count > 50000:
                    root_print(
                        ...
                        f"          - move_size_rotation too large or too small\n"
            ...
            phi_after_relax = packing_fraction(N, particle_volume, sim.state.box)
        ...
        root_print(
            ...
            f"              Current phi: {packing_fraction(N, particle_volume, sim.state.box):.6f}\n"
    ...
    phi_final = packing_fraction(N, particle_volume, sim.state.box)
...
def write_final_outputs(
    ...
    Args:
        ...
        mc (hoomd.hpmc.integrate.ConvexPolyhedron): HPMC integrator with final stats.
    ...
    _write_snapshot_with_shape(sim, compressed_gsd, mc.type_shapes)
    root_print(f"[OUTPUT] Labelled snapshot => {compressed_gsd}")

    # Canonical final state
    # Standard filename for multi-stage chaining
    _write_snapshot_with_shape(sim, files.final_gsd, mc.type_shapes)
    root_print(f"[OUTPUT] Final GSD         => {files.final_gsd}")

    _verify_gsd_type_shapes(files.traj_gsd, params.shape_vertices, label="trajectory")
    _verify_gsd_type_shapes(files.restart_gsd, params.shape_vertices, label="restart")
    ...
    summary = {
        ...
        "particle_volume": params.particle_volume,
        "n_shape_vertices": len(params.shape_vertices),
...
def _write_snapshot_with_shape(sim: hoomd.Simulation,
    ...
    """
    Write a single-frame convex-polyhedron GSD with particle orientations and
    particle-type shape metadata explicitly stored.

    This helper is used for:
      - the labelled compressed snapshot
      - the canonical final snapshot
      - emergency snapshots
      - single-frame checkpoint snapshots

    The stored type_shapes metadata makes the file self-describing for
    visualization tools such as OVITO.
    """
    ...
    if getattr(snap.particles, "orientation", None) is None:
        sys.exit(
    ...
    if getattr(snap.particles, "moment_inertia", None) is not None:
        frame.particles.moment_inertia = np.asarray(
            snap.particles.moment_inertia, dtype=np.float32
        )
...
def _verify_gsd_type_shapes(filename: str,
    ...
    """
    Verify that a written GSD contains convex-polyhedron shape metadata and that
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Usage: python3 convex_polyhedron_compress.py --simulparam_file simulparam.json"
        ),
    ...
    _print_banner("HOOMD-blue v4 | Convex-Polyhedron HPMC Compression", width=80)
    ...
    try:
        ...
        root_print(
            ...
            f"  Current phi:           {packing_fraction(N, params.particle_volume, sim.state.box):.6f}\n"
        ...
        try:
            ...
            _write_snapshot_with_shape(sim, emergency_filename, mc.type_shapes)
            root_print(
    ...
    phi_final = packing_fraction(N, params.particle_volume, sim.state.box)
